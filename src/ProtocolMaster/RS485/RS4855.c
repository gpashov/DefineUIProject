...
/* VARIABLES */
/* ------------------------------------------------------------------------------------------ */
LDD_TDeviceData *TX_ENABLE_struct;




UART_nameTypeDef ptUART_CH[MAX_CHANNEL_RS485] = RS485_UART;
...
{
	RS485_TRANSMIT_ENABLE_DIRECTION(u8Channel);
    ...
    switch(xEstat485[u8Channel].u8EstatBusInter)
    {
    	case 0:
			...
			case 1:
				...
				{
					case SUCCESS: xEstat485[u8Channel].u32Timer = TICKS_GetTicks(); xEstat485[u8Channel].u8EstatBusInter++; break;
					case ERROR: xEstat485[u8Channel].u8EstatBusInter = 6; break;
					default: break;
				}
					break;
			case 2:
					...
					else if(TICKS_DiffTicks(xEstat485[u8Channel].u32Timer) > 500)
					{
						xEstat485[u8Channel].u8EstatBusInter = 6;
					}
					break;
